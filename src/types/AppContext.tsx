    ...
    switch (action.type) {
        ...
        case 'LOGIN_SUCCESS':
            ...
            return {
                ...state,
                user: user,
                isAdmin: user.role === 'admin', // Правильно определяем isAdmin по роли
...
export const AppProvider: React.FC<AppProviderProps> = ({ children }) => {
    ...
    const register = async (data: RegisterData): Promise<RegisterResult> => {
        try {
            ...
            // Создаем нового пользователя (ВСЕГДА role: 'user')
            const newUserId = Date.now();
            const newUser: User = {
                id: newUserId,
                email: data.email,
                name: data.fullName,
                role: 'user', // ВАЖНО: только user при обычной регистрации!
                avatar: '/images/user-avatar.png',
                phone: data.phone
            };
    ...
    const registerAdmin = async (data: AdminRegisterData): Promise<RegisterResult> => {
        // Только админы могут создавать новых админов
        if (!state.isAdmin) {
            return {
                success: false,
                error: 'Только администраторы могут создавать новых администраторов'
            };
        }
